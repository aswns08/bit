  ...
  public void doPost(
    ...
    try {
      ...
      if(member != null) {
    	  	...
    	  	response.sendRedirect("../product/list.do"); // product/list.do 를 요청해라
    	  	
      } else {
    	  session.invalidate(); // session을 제거하고 새로 만든다.
    	  response.sendRedirect("login.do"); //  product/list.do 를 요청해라
      }
