...
=> ServletRequest(request), HttpSession(session), 
   ServletContext(application), PageContext(page)로 부터
  객체 꺼내기 또는 객체 생성.
  
속성
id => 레퍼런스 변수.
scope => 객체를 꺼낼 보관소 지정. page, request, session, application
class => 보관소에서 꺼낼 객체의 타입. 꼬는 객체 생성을 위한 타입.
type => 레퍼런스 변수의 타입.
...
scope="page"/>
 ...
 객체를 새로 생성하지 못ㅎ나다. 오류 발생 !</p>
  
<%-- class를 type으로 바꿔서 에러 확인. --%>
<jsp:useBean 
class="java63.servlets.test05.domain.Product"  
id="product3"
scope="page"/>
 <%--
 Product product3 = (Product)pageContext.getArrtibute("product3");
 if(product3 == null) { // 객체 못찾으면 오류 !
  throw new InstantiationException 오류!
}
  --%>
