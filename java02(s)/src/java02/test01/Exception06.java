/* 예외 발생을 먼 호출자에게 던지고자 할 때
 ...
 - 아니 강사님 그러면, 그냥 Error 사용하면 안됩니까??
   => Error는 시스템 오류를 담을 때 사용한다.
   => 따라서 애플리케이션에서 Error을 사용하는 것은 바람직 하지 않다.
   => 그럼에도 불구하고 Error처럼 예외를 던지고 싶을때가 있는데
      그럴 경우 대비해서 만든 클래스가 RunTimeException 이다.
 */
...
public class Exception06 {
  ...
  static String op;
    ...
    static class Calculator {
      ...
      // 반드시 메서드 선언부에 throws Exception 처럼 지정해야 한다.
    ...
    // compute()에서 발생하는 에외를 처리하기 싫으면 상위 호출자에게 넘긴다.
